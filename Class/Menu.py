...
class Menu:
    ...
    def run(self):
        ...
        while True:
            ...
            for i in range(len(MENU_OPTION)):
                if i == menu_opition:
                    self.Menu_text(30, MENU_OPTION[i], COLOR_YELLOW, ((
                        WIN_WIDTH / 2), 200 + (i * 25)))
                else:
                    self.Menu_text(30, MENU_OPTION[i], COLOR_WHITE, ((
                        WIN_WIDTH / 2), 200 + (i * 25)))
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_DOWN:
                        menu_opition = menu_opition + \
                            1 if menu_opition < len(MENU_OPTION) - 1 else 0
                    if event.key == pygame.K_UP:
                        menu_opition = menu_opition - \
                            1 if menu_opition > 0 else len(MENU_OPTION) - 1
    ...
    def Menu_text(self, text_size: int, text: str, text_color: tuple, text_center_pos: tuple):
        text_font: Font = pygame.font.SysFont(
            "Lucida Sans Typewriter", text_size)
